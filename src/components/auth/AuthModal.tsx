...
export default function AuthModal({ isOpen, onClose, onSuccess }: AuthModalProps) {
  ...
  // Set mounted state for Next.js Portal rendering
  useEffect(() => {
    setMounted(true);
  ...
  // Do not render anything during SSR or before mount
  ...
  return createPortal(
    <div 
      ...
      style={{ zIndex: 9999 }}
    >
      {/* 2. Full-screen backdrop: rgba(0,0,0,0.65) + 12px blur */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      ...
      {/* 4. Premium Glassmorphism Card: 520px max-width, 24px blur, custom shadows */}
      <motion.div
      ...
      >
        ...
        <div className="flex flex-col items-center text-center py-4">
          ...
          <h2 className="text-xl md:text-2xl font-bold font-space-grotesk text-text-primary tracking-tight">
            Sign in to Dev-Track
          </h2>
          <p className="mt-3 text-sm text-text-secondary leading-relaxed max-w-sm">
            Connect your account to analyze your coding consistency, calculate your developer metrics, and unlock personalized career recommendations.
          </p>
          ...
          <button
            onClick={handleLogin}
            disabled={loading}
            className="mt-8 flex w-full items-center justify-center gap-3 rounded-lg bg-[#F0F6FC] px-4 py-3 text-sm font-semibold text-[#0D1117] hover:bg-[#E0E6EC] active:scale-[0.98] transition-all disabled:opacity-50 disabled:pointer-events-none"
          >
            {loading ? (
              <>
